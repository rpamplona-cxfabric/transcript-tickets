...
export const TableHeader = ({ children, sticky = false }: TableHeaderProps) => (
  <thead
    className={`app-surface border-b border-zinc-200 text-[11px] font-semibold uppercase tracking-wide text-zinc-700 dark:border-zinc-800 dark:text-zinc-300 ${sticky ? "sticky top-0 z-10" : ""}`}
  >
    {children}
  </thead>
);
  ...
  >
    ...
    >
      ...
      <span className="relative h-3.5 w-3 shrink-0" aria-hidden="true">
        <ChevronUp
          className={`absolute -top-0.5 left-0 h-3 w-3 ${sortDirection === "asc" ? "text-current" : "text-zinc-400/70 dark:text-zinc-600"}`}
        />
        <ChevronDown
          className={`absolute -bottom-0.5 left-0 h-3 w-3 ${sortDirection === "desc" ? "text-current" : "text-zinc-400/70 dark:text-zinc-600"}`}
        />
      </span>
    </button>
  </th>
