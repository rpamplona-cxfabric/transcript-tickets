...
export const LeadModal = ({ isOpen, onClose, prefillName, transcriptId, onSuccess }: LeadModalProps) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/40 backdrop-blur-xs p-4 animate-fade-in">
      <div
        onClick={(e) => e.stopPropagation()}
        className="app-surface app-shadow-surface w-full max-w-md rounded-2xl border border-zinc-200 p-6 dark:border-zinc-800 flex flex-col gap-4 animate-scale-up"
      >
        <div className="flex items-center justify-between border-b border-zinc-150 pb-3 dark:border-zinc-850">
          ...
          <button onClick={onClose} className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-900 dark:hover:bg-black dark:hover:text-white cursor-pointer">
