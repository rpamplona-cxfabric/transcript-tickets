...
export const TranscriptionDetailDrawer = () => {
      ...
      <div className="space-y-4">
        {text.split('\n').map((line, idx) => {
          ...
          if (parsedLine) {
            ...
            return (
              <div key={idx} className="app-surface-raised flex flex-col gap-1 rounded-lg p-3 border border-zinc-100 dark:border-[#293442]">
                <div className="flex items-center gap-2 text-xs lg:text-sm font-semibold text-zinc-500">
                  ...
                  {timeTag && <span className="bg-zinc-200 dark:bg-black px-1.5 py-0.5 rounded text-[10px]">{timeTag}</span>}
                </div>
                <p className="text-sm text-zinc-800 dark:text-zinc-300">{speakerText.trim()}</p>
              </div>
            );
          }
          return <p key={idx} className="app-surface-raised rounded p-2 text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">{line}</p>;
        })}
      </div>
    );
  };
  ...
  return (
    <>
      <div onClick={() => setActiveTranscript(null)} className="fixed inset-0 z-[80] bg-zinc-950/30 backdrop-blur-xs transition-opacity duration-300 cursor-pointer" />
      ...
      <div className="app-surface app-shadow-surface fixed inset-y-0 right-0 z-[90] flex h-[100svh] w-full flex-col transition-transform duration-300 border-l border-zinc-200 dark:border-zinc-800 md:max-w-3xl">
        <div className="flex h-16 shrink-0 items-center justify-between border-b border-zinc-200 px-4 sm:px-6 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => downloadTextFile(activeTranscript)} title="Download Transcript" className="rounded-lg p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-black dark:hover:text-white cursor-pointer animate-fade-in">
              <Download className="h-5 w-5" />
            </button>
            <button onClick={() => setActiveTranscript(null)} className="rounded-lg p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-black dark:hover:text-white cursor-pointer">
              <X className="h-5 w-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          {!activeTranscript.isProcessed && !activeTranscript.isIgnored && (
            <div className="app-surface-raised space-y-3 rounded-xl border border-zinc-200 p-5 dark:border-[#293442]">
              <div>
                ...
                <p className="text-xs lg:text-sm text-zinc-500 dark:text-zinc-400 font-medium">Search for an existing lead or create a new one.</p>
              </div>
          ...
          <div className="app-surface-raised space-y-4 rounded-xl border border-zinc-200 p-5 dark:border-[#293442]">
            <div>
              ...
              <p className="text-xs lg:text-sm text-zinc-500 dark:text-zinc-400 font-medium">Map generic speakers to their real names.</p>
            </div>
            {speakers.length > 0 ? (
              <div className="space-y-3">
                {speakers.map((speaker) => (
                  <div key={speaker} className="flex flex-col gap-2 border-b border-zinc-100 py-2 text-sm last:border-0 dark:border-zinc-800 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                    <span className="text-xs lg:text-sm font-bold text-zinc-650 dark:text-zinc-400">{speaker}</span>
                    <SpeakerCombobox
                      speaker={speaker}
              ...
              <p className="text-xs lg:text-sm italic text-zinc-500 dark:text-zinc-400 font-medium">No speakers identified in this transcript.</p>
            )}
          </div>
      ...
      {isIgnoreDialogOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          />
          <div role="dialog" aria-modal="true" aria-labelledby="ignore-transcription-title" className="app-surface app-shadow-surface relative w-full max-w-md rounded-2xl border border-zinc-200 p-6 dark:border-zinc-800">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={() => setIsIgnoreDialogOpen(false)} disabled={isIgnoring} className="rounded-lg px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-100 disabled:opacity-60 dark:text-zinc-300 dark:hover:bg-black">Cancel</button>
