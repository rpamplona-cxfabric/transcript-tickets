...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (transcript.speakerNames) {
      for (const [genericName, mappedName] of Object.entries(
        transcript.speakerNames,
      )) {
        if (mappedName) {
          const escapedGeneric = genericName.replace(
            /[-/\\^$*+?.()|[\]{}]/g,
            "\\$&",
          );
          const regex = new RegExp(
            `(^|\\n|\\r)(\\[.*?\\])?\\s*${escapedGeneric}\\s*:`,
            "g",
          );
          mappedTranscript = mappedTranscript.replace(
            regex,
            `$1$2 ${mappedName}:`,
          );
        }
    ...
    if (!webhookUrl) {
      console.warn(
        "FLOW_A_WEBHOOK_URL environment variable is not set. Task generation will proceed in mock mode.",
      );
      return NextResponse.json({
        success: true,
        message:
          "Task generation triggered (mock mode - FLOW_A_WEBHOOK_URL not configured)",
        mappedTranscript,
        leadName,
        leadId,
      });
    ...
    if (!processorBearer) {
      console.error(
        "TRANSCRIPT_PROCESSOR_BEARER environment variable is not set.",
      );
      return NextResponse.json(
    ...
    console.error(
      "API Error in POST /api/transcriptions/generate-tasks:",
      error,
    );
    return NextResponse.json(
